...
- 退出过滤（新建钩子，现有无专用点）：退出游戏时后台静默记三类结论——玩了什么/到哪（进度）、喜恶偏好（画像），无弹窗确认，只存结论条目、事后可查看可删；挂点仿 `DateSession` 式 beforeunload/visibility/onExit（`components/date/DateSession.tsx:856`、`apps/DateApp.tsx:653`）；会话开关关闭时整条链路不跑。
