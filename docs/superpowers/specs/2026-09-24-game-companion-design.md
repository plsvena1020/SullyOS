...
3. WASAPI+VAD+预生成+压低/暂停全套音频（三档只留纯文本一档）。
...
- 游戏说话时不播但后台预生成 1 条（新覆盖旧，OCR 位移即作废）。
- VAD 静音 hangover 800–1500ms 确认真空档再播；平板无音频时用字幕稳定 2 秒代替（常量可配）。
- 伴侣说到一半游戏开口：立刻压低/暂停，短句丢弃。
...
- Phase 2：音频链路（预生成+确认再播+打断）与三档切换。
