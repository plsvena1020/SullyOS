...
export const CALL_REGISTRY: CallSite[] = [
    // ── 主聊天链路 10 ──
    { site: 'chat-main', category: '主聊天链路', name: '主聊天请求', blurb: 'useChatAI 主请求 + 重试 + 工具轮续跑', trigger: '发送前', sources: ['chat.steelExpression', 'chat.steelYourself', 'voice.minimax', 'voice.fish', 'voice.elevenlabsV3', 'voice.elevenlabsStd', 'chat.perspectiveTool'], gates: [G.api], visibility: 'local', anchor: 'hooks/useChatAI.ts:1063', pin: 'baseReqBody' },
    ...
    // ── 记忆系统 7 ──
    { site: 'memory-extract', category: '记忆系统', name: '记忆提取', blurb: 'extraction：话题盒记忆提取', trigger: '每轮回复后', sources: ['memory.extractionRules', 'memory.extractionEntityRule', 'memory.extractionMain'], gates: [G.api], visibility: 'local', anchor: 'utils/memoryPalace/extraction.ts:409', pin: 'safeFetchJson' },
    { site: 'memory-digest', category: '记忆系统', name: '记忆消化', blurb: 'digestion 反刍：独处反思任务', trigger: '每轮回复后', sources: ['memory.reflectTask'], gates: [G.api], visibility: 'local', anchor: 'utils/memoryPalace/digestion.ts:334', pin: 'safeFetchJson' },
    { site: 'personality-detect', category: '记忆系统', name: '认知风格判定', blurb: '消化内的角色认知风格与反刍倾向判定', trigger: '每轮回复后', sources: ['memory.personalityDetect'], gates: [G.api], visibility: 'local-uncaptured', anchor: 'utils/memoryPalace/digestion.ts:979', pin: 'safeFetchJson' },
    ...
    { site: 'song-mentor', category: '创作功能', name: '写歌导师', blurb: '写歌导师系统提示 + 轮次点评', trigger: '发送前', sources: ['song.craftRules'], gates: [G.api], visibility: 'local', anchor: 'apps/SongwritingApp.tsx:501', pin: 'chat/completions' },
    ...
    { site: 'settings-probe', category: '创作功能', name: '设置探针', blurb: 'Settings API 连通性探测', trigger: '手动按钮', sources: ['hardcoded:apps/Settings'], gates: [G.manual], visibility: 'local-uncaptured', anchor: 'apps/Settings.tsx:2589', pin: 'chat/completions' },
    { site: 'festival-lines', category: '创作功能', name: '节日台词', blurb: '今日特殊节日兜底行：prompt 内硬编码注入，非独立 LLM 调用', trigger: '发送前', sources: ['hardcoded:utils/chatPrompts'], gates: [G.api], visibility: 'local-uncaptured', anchor: 'utils/chatPrompts.ts:565', pin: '今日特殊节日' },
    ...
    // ── 工具与识图 7 ──
    { site: 'tool-recall', category: '工具与识图', name: 'RECALL / SEARCH 二轮', blurb: 'agenticTools：记忆调取工具声明与执行', trigger: '工具轮内', sources: ['hardcoded:utils/agenticTools'], gates: [G.inTurn], visibility: 'local-uncaptured', anchor: 'utils/agenticTools.ts:163', pin: 'RECALL' },
