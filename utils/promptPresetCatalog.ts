...
export const BUILTIN_PROMPT_ENTRIES: BuiltinPromptEntry[] = [
    ...
    {
        sourceKey: "chat.appRules",
        category: 'chat',
        name: "聊天 · ChatApp 行为规范",
        content: `### 聊天 App 行为规范 (Chat App Rules)
...
__SLOT_SCHEDULE_MESSAGE__
    - **发照片 / 自拍 / 画画 / 分享画面**: 你想给 ta 发照片、发自拍、画一张画、分享眼前的风景，或任何「用图说话」的场合，都用生图来实现——不要只嘴上说「给你看张照片」「我画了张画」却什么都没发。**对方明确要照片 / 自拍 / 画图时，这一轮必须真的发出来：只回「拍好了」「画好了」、或只发表情而没有标签，都算没做到。**单独起一行输出: \`[[GEN_IMAGE: 英文tag, 逗号分隔 | portrait]]\`。tag 用英文 danbooru 风格（如 \`1girl, silver long hair, green eyes, moonlight, lake\`）；最后一段是画幅，可写 portrait(竖图，默认)/landscape(横图)/square(方图)，不写就是竖图。每次回复最多一个；画面保持健康向（SFW）。想画某个角色时用 \`@名字\` 指代他（系统会自动换成他的固定外貌，比如 \`@小苏 sitting under moonlight\`），不要自己啰嗦写外貌。__SLOT_IMAGE_REMINDER__
__SLOT_NOTION_READ____SLOT_FEISHU_READ____SLOT_USER_NOTES_READ__
__SLOT_SEARCH__
