...
describe('DatePrompts.buildSessionPayload', () => {
    ...
    it('sceneBlock 非空且时间感知开启时插在场景上下文区、观测块之前', async () => {
        const scene = '时间：2026-01-02 11:04（Asia/Shanghai）\n地点：公司楼下咖啡店\n近期行踪：\n- 刚从公司出来';
        const char = makeChar({ dateObserve: { enabled: true } });
        const sys = sysOf((await DatePrompts.buildSessionPayload({ ...baseInput(char), sceneBlock: scene })).messages);
    ...
    it('时间感知关闭时 sceneBlock 整段撤掉；缺省/空串/纯空白逐字节不变', async () => {
        const scene = '时间：2026-01-02 11:04（Asia/Shanghai）\n地点：公司楼下咖啡店\n近期行踪：\n- 刚从公司出来';
        // 关掉时间感知：system 里没有会随时钟漂移的行，且实时处境必须整段消失
        const char = makeChar({ dateTimeAwarenessEnabled: false, dateObserve: { enabled: true } });
        const withScene = sysOf((await DatePrompts.buildSessionPayload({ ...baseInput(char), sceneBlock: scene })).messages);
        ...
        expect(withScene).not.toContain('### 实时处境');
        expect(withScene).not.toContain('公司楼下咖啡店');
        expect(withScene).toBe(absent);
...
describe('DatePrompts.buildPeekPayload', () => {
    ...
    it('sceneBlock 非空且时间感知开启时进入「场景：感知」区', () => {
        const scene = '时间：2026-01-02 11:04（Asia/Shanghai）\n地点：公司楼下咖啡店';
        const char = makeChar();
        const withScene = DatePrompts.buildPeekPayload({ char, userProfile: user, allMsgs: [makeMsg({ timestamp: 1700000000000 })], emojis: [], sceneBlock: scene });
    ...
    it('时间感知关闭时 sceneBlock 整段撤掉；缺省/空串逐字节不变', () => {
        const scene = '时间：2026-01-02 11:04（Asia/Shanghai）\n地点：公司楼下咖啡店';
        ...
        const userMsg = withScene.messages[withScene.messages.length - 1].content as string;
        expect(userMsg).not.toContain('### 实时处境');
        ...
        expect(JSON.stringify(withScene.messages)).toBe(JSON.stringify(absent.messages));
