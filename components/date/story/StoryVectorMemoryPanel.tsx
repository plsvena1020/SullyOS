...
const StoryVectorMemoryPanel: React.FC<Props> = ({ entry, onBack }) => {
    ...
    return <div className='h-full w-full flex flex-col bg-stone-100 text-slate-800 animate-page-in-l'>
        ...
        <BottomSheet open={!!menuNode} onClose={() => setMenuNode(null)} overlayClassName="z-[80] bg-slate-950/35" panelClassName="story-safe-sheet sm:max-w-sm rounded-t-[28px] bg-stone-100 px-5 pt-5 shadow-2xl">{menuNode && (<><div className='flex items-center justify-between'><div><div className='text-[9px] uppercase tracking-[.2em] text-violet-500 font-bold'>Memory action</div><h3 className='mt-1 font-semibold'>这条剧情记忆</h3></div><button onClick={() => setMenuNode(null)} className='w-9 h-9 rounded-full bg-white border border-slate-200 grid place-items-center'><X size={15} /></button></div><div className='mt-5 border-t border-slate-200'><button onClick={() => openEdit(menuNode)} className='w-full py-4 flex items-center gap-3 text-sm font-semibold'><PencilSimple size={18} className='text-violet-500' />编辑文字并重建向量</button><button onClick={() => { setDeleting(menuNode); setMenuNode(null); }} className='w-full py-4 border-t border-slate-200 flex items-center gap-3 text-sm font-semibold text-rose-600'><Trash size={18} />删除这条记忆</button></div></>)}</BottomSheet>

        <BottomSheet open={!!editing} onClose={() => { if (!saving) setEditing(null); }} dismissible={!saving} overlayClassName="z-[80] bg-slate-950/35" panelClassName="story-safe-sheet story-keyboard-sheet sm:max-w-lg rounded-t-[28px] bg-stone-100 px-5 pt-5 shadow-2xl"><div className='flex items-start justify-between gap-3'><div><div className='text-[9px] uppercase tracking-[.2em] text-violet-500 font-bold'>Re-embed</div><h3 className='mt-1 font-semibold'>编辑剧情记忆</h3><p className='mt-1 text-[10px] text-slate-500'>正文变化后会沿用原 ID 重建向量，不会产生重复记忆。</p></div><button disabled={saving} onClick={() => setEditing(null)} className='w-9 h-9 rounded-full bg-white border border-slate-200 grid place-items-center'><X size={15} /></button></div><textarea autoFocus value={draft} onChange={event => setDraft(event.target.value)} className='mt-5 w-full min-h-40 resize-none rounded-2xl border border-slate-200 bg-white p-4 text-sm leading-6 outline-none focus:border-violet-400' /><button disabled={saving || !draft.trim()} onClick={() => void saveEdit()} className='mt-4 w-full h-12 rounded-2xl bg-slate-900 text-white font-semibold disabled:opacity-40'>{saving ? <SpinnerGap size={18} className='mx-auto animate-spin' /> : '保存并同步向量'}</button></BottomSheet>

        <BottomSheet open={!!deleting} onClose={() => { if (!saving) setDeleting(null); }} dismissible={!saving} overlayClassName="z-[80] bg-slate-950/35" panelClassName="story-safe-sheet sm:max-w-sm rounded-t-[28px] bg-stone-100 px-5 pt-5 shadow-2xl"><div className='text-[9px] uppercase tracking-[.2em] text-rose-500 font-bold'>Delete memory</div><h3 className='mt-1 text-lg font-semibold'>删除后无法恢复</h3><p className='mt-2 text-[11px] leading-5 text-slate-500'>只会删除当前剧情分区内的这一条节点、向量和关联边。</p><div className='mt-5 grid grid-cols-2 gap-3'><button disabled={saving} onClick={() => setDeleting(null)} className='h-11 rounded-2xl bg-white border border-slate-200 text-xs font-bold'>取消</button><button disabled={saving} onClick={() => void confirmDelete()} className='h-11 rounded-2xl bg-rose-600 text-white text-xs font-bold'>{saving ? '删除中…' : '确认删除'}</button></div></BottomSheet>
    </div>;
};
